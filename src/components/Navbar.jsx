...
const Navbar = forwardRef(
  (
    ...
    const handleKeyPress = (e) => {
      if (e.key === "Enter") {
        ...
        setActiveIndex((prevIndex) =>
          prevIndex < filteredMmiIds.length - 1 ? prevIndex + 1 : prevIndex
        );
    ...
    return (
      <div
        ref={ref}
        className="w-full h-[70px] bg-purple-950 text-white flex items-center justify-between px-10 font-ruda"
      >
        ...
        <div className="text-center flex-grow">
          {selectedMmiId && (isDetailedDashboard || isDriverStat) && (
            <span className="text-xl font-semibold text-white mmid-show">
              Showing stats for MMI ID:{" "}
              <span className="text-yellow-400">{selectedMmiId}</span>
            </span>
          )}
        </div>
